...
export function AnalysisPage({ id }: { id: string }) {
  ...
  const actionTitle = !hasBias
    ? 'NO CLEAR DIRECTION'
    : entryReady
      ? `${a.direction === 'buy' ? 'BUY' : 'SELL'} SETUP READY`
      : `${a.direction === 'buy' ? 'BUY' : 'SELL'} BIAS — WAIT FOR ${a.direction === 'buy' ? 'LONG' : 'SHORT'} ENTRY`;
    ...
    <div className="min-h-screen animate-fade-in">
      ...
      <header className="sticky top-0 z-30 border-b border-white/[0.06] bg-ink-950/90 backdrop-blur-xl">
        <div className="mx-auto flex max-w-[1200px] items-center justify-between px-5 py-3">
          ...
          {entryReady && hasBias && <button onClick={saveToJournal} disabled={savedToJournal} className={savedToJournal ? 'btn-outline' : 'btn-neon'}>{savedToJournal ? <><BookOpen size={15} /> Saved</> : <><BookOpen size={15} /> Log trade</>}</button>}
        </div>
      </header>

      <main className="mx-auto max-w-[1200px] px-5 py-6 pb-24">
        <GlassCard className={`overflow-hidden border ${!hasBias ? 'border-warn-500/30' : a.direction === 'buy' ? 'border-neon-500/30' : 'border-bear-500/30'}`}>
          <div className="grid gap-6 p-6 md:grid-cols-[1fr_auto] md:p-8">
            <div>
              ...
              <h1 className={`mt-2 font-display text-4xl font-800 tracking-tight sm:text-5xl ${!hasBias ? 'text-warn-400' : a.direction === 'buy' ? 'text-neon-400' : 'text-bear-400'}`}>{actionTitle}</h1>
        ...
        <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_340px]">
          <div className="space-y-5">
            {a.image_url && a.overlays ? <ChartOverlay imageUrl={a.image_url} overlays={a.overlays} entry={!entryReady ? null : a.entry} stopLoss={!entryReady ? null : a.stop_loss} takeProfit={!entryReady ? null : a.take_profit} /> : <GlassCard className="overflow-hidden">{a.image_url && <img src={a.image_url} alt={a.symbol} className="max-h-[560px] w-full bg-ink-900 object-contain" />}</GlassCard>}
