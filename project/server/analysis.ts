...
Base every claim on visible evidence. Do not invent exact indicator readings when an indicator is not visible, do not predict an unseen future crash or spike, and do not claim certainty from a static screenshot. If the chart is unclear or lacks a valid entry, use direction "neutral", conservative scores, and explain the specific evidence that is missing or risky. Return buy or sell when at least two independent visible factors support that direction, the entry has a defensible invalidation level, and reward-to-risk is acceptable. A directional recommendation normally requires confidence >= 65, trend_strength >= 40, momentum_score >= 40, risk_score <= 72, reward-to-risk >= 1.3, and internally consistent levels. On M1-M5 and synthetic instruments use the moderately stricter server thresholds, but do not reject a setup solely because it is a Crash or Boom index. If a recent oversized impulse has no visible retracement or confirmation, direction MUST be neutral. Price levels must come from the visible price scale and remain internally consistent with the direction. Scores must vary with the evidence rather than defaulting to a habitual mid-range value; the server independently calibrates final confidence. Overlay coordinates are normalized from 0 to 1. This is educational decision support, not financial advice.` },
